...
const common_test = function () {
    ...
    this.__checkElement_isDisplayed = (obj, bDisplayedTrue_NotDisplayedFalse = true) => {
        if (bDisplayedTrue_NotDisplayedFalse) {
            fcommon_obj.__ElementPresent('', obj)
    ...
    this.__checkElement_attributeValue = (obj, attribute, text) => {
        fcommon_obj.__ElementPresent('', obj)
        expect(obj.getAttribute(attribute)).toContain(text)
    }
    ...
    this.__clickArticleTitle = (sTitle) => {
        fcommon_obj.__executeScript(element(by.cssContainingText('a', sTitle)), sTitle, "click");
        pcommon_page.__wait4PageReady('', pnews_details_page._cardHeader)
    }

    this.__checkInternalOnlyAttachment = (obj) => {
        fcommon_obj.__ElementPresent('', obj)
        expect(obj.getAttribute('href')).toContain('https://docs.src.us-east-1.dev.int.mercer.com/v1/api/proxy/document/')
    }

    this.__checkNormalAttachment = (obj) => {
        fcommon_obj.__ElementPresent('', obj)
        expect(obj.getAttribute('href')).toContain('https://src.us-east-1.dev.awsapp.mercer.com/v1/api/uploads/')
    }
    ...
    this.__checkInternalOnlyArticle_backGroundColor = (obj) => {
        fcommon_obj.__ElementPresent('', obj)
        expect(obj.getAttribute('class')).toContain('src-c-content-list-card--internal')
        expect(obj.getCssValue('background-color')).toEqual('rgba(215, 233, 255, 1)')
    }

    this.__checkInternalOnlyPopularArticle_backGroundColor = (obj) => {
        fcommon_obj.__ElementPresent('', obj)
        expect(obj.getAttribute('class')).toContain('src-c-countable-item-list__item-content--internal')
        expect(obj.getCssValue('background-color')).toEqual('rgba(215, 233, 255, 1)')
    }


    this.__checkNormalArticle_backGroundColor = (obj) => {
        fcommon_obj.__ElementPresent('', obj)
        expect(obj.getAttribute('class')).not.toContain('src-c-content-list-card--internal')
        expect(obj.getCssValue('background-color')).toEqual('rgba(0, 0, 0, 0)')
    }

    this.__checkNormalPopularArticle_backGroundColor = (obj) => {
        fcommon_obj.__ElementPresent('', obj)
        expect(obj.getAttribute('class')).not.toContain('src-c-countable-item-list__item-content--internal')
        expect(obj.getCssValue('background-color')).toEqual('rgba(0, 0, 0, 0)')
    }
    ...
    let __GivenUserSubmitDraftAnEvent = function (td, bSubmitTrue_SaveDraftFalse = true) {


        describe('GIVEN: ' + td.User + ' submit / draft an Event with all fields filled: ' + td.EventName, function () {
            ...
            it('User goes to Publish -> Event and fill all fields', function () {
                ...
                fcommon_obj.__setText('_txtEventName', pcommon_publish._txtEventName, td.EventName);
            ...
            it('User uploads Featured Image: ' + td.Upload_FeaturedImage, function () {
                browser.getCapabilities().then(function (txt) {
    ...
    this.__GivenUserSubmitAnEvent = function (td, bSubmitTrue_SaveDraftFalse = true) {

        __GivenUserSubmitDraftAnEvent(td, bSubmitTrue_SaveDraftFalse);


    };


    this.__GivenUserSubmitAnArtical = function (td, bResearchTrue_NewsFalse = true) {


        describe('GIVEN: ' + td.User + ' publishespublishes an artical with all fields filled: ' + td.Title, function () {
            ...
            it('User clicks Uploads Attachemts dropzone', function () {
                browser.getCapabilities().then(function (txt) {
                    if (txt.get('browserName') !== 'MicrosoftEdge') {
            ...
            it('User uploads Attachemts: ', function () {
    ...
    this.__SubmitAndAddAnotherArticle = (td) => {
        switch (td.ArticleType) {
            case 'Research':
                pcommon_publish.__SubmitAndAddAnother(td.Research_Submit_SuccessMsg);
                break
            case 'News':
                pcommon_publish.__SubmitAndAddAnother(td.News_Submit_SuccessMsg);
                break
            case 'Event':
                pcommon_publish.__SubmitAndAddAnother(td.Event_Submit_SuccessMsg);
                break
        }
    }
    ...
    this.__ArticlePublish = (td) => {
        ...
        //set Video Link
        if (td.VideoLink != null && td.VideoLink != '' && td.ArticleType === 'Research') {
    ...
    this._txtSearchArticle = element(by.css('[id="mos-autocomplete-1"]'))

    this._lstSearchArticleResult = element(by.css('[src-c-selected-filters-bar mos-u-grid-full-width ng-star-inserted"]'))
    this._txtSearchArticleResult = (sText) => {
        return element(by.cssContainingText('.mos-u-spacer--margin-right-sm ng-star-inserted', sText))
    }

    this.__SearchArticle = (sText) => {
        fcommon_obj.__executeScript(this._txtSearchArticle(sArticleTitle), "_txtSearchArticle", "click")
        fcommon_obj.__setText("_txtSearchArticle", this._txtSearchArticle, sText)


    }

    /////////////////////////////  lin: open articles   /////////////////////////
    this._cardArticle = element.all(by.css('[class^="src-c-content-list-card mos-u-spacer--padding-top-bottom-md mos-u-border--bottom-sm mos-u-color-border--accent2"]'))

    this._lnkArticle = (sArticleTitle) => {
        return this._cardArticle.filter((elem, index) => {
            return elem.element(by.css('a[href$=' + sArticleTitle + ']'))
        }).first()
    }

    this._cardPopularArticle = element.all(by.css('[class^="src-c-countable-item-list__item-content mos-u-spacer--padding-top-lg mos-u-spacer--padding-left-xlg"]'))

    this._lnkPopularArticle = (sArticleTitle) => {
        return this._cardPopularArticle.filter((elem, index) => {
            return elem.element(by.css('a[href$=' + sArticleTitle + ']'))
        }).first()
    }

    this.__OpenArticle = (sArticleTitle) => {
        fcommon_obj.__executeScript(this._lnkArticle(sArticleTitle), "_lnkArticle", "click");
        expect(element(by.css('[class="src-c-article-header__title mos-u-spacer--margin-none mos-u-color-text--white"]')).getText()).toBe(sArticleTitle)
    }

    this.__OpenPopularArticle = (sArticleTitle) => {
        fcommon_obj.__executeScript(this._lnkPopularArticle(sArticleTitle), "_lnkPopularArticle", "click");
        expect(element(by.css('[class="src-c-article-header__title mos-u-spacer--margin-none mos-u-color-text--white"]')).getText()).toBe(sArticleTitle)
    }
