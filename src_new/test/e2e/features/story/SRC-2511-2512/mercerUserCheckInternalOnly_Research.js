...
let research_details_page = require('../../../page-objects/research_details');
let presearch_details_page = new research_details_page();
...
let td_content = {
    ...
    "PostsType": td_posts.research,

    "InternalOnly": true,

    "Title": '2512_AutoResearch_internalOnly_' + futil_timer.__returnYYYYMMDDHMS(),
    ...
    "Research_Submit_SuccessMsg": 'Thank you for your research article submission. Please note that all submissions are sent to moderation for admin approval. You will be notified when your submission is approved.',
    ...
    "VideoLink_embed": 'https://www.youtube.com/embed/haSX6qLGA3w',
};

let popularArticle_content = {

    "title": 'Research_internalOnly_notRemove_2512',
    "company": 'Mercer',
    "readTime": '8',
    "Type": 'House View',
    "ExecutiveSummary": "This is ExecutiveSummary: Research_internalOnly_notRemove_2512"
...
describe('Verify Mercer Admin check Internal Only ' + td_content.ArticleType, function () {
    ...
    describe('Verify Mercer Admin check Internal Only ' + td_content.ArticleType + ' on ' + td_content.ArticleType + ' page', function () {
        ...
        it('AND: I see the internal only ' + td_content.ArticleType + ' card with a tag is: ' + td_content.Type.toUpperCase(), function () {
            fcommon_test.__checkElement_getText(presearch_page._tag_inArticleCard(td_content.Title), td_content.Type.toUpperCase())
        })
    ...
    describe('Verify Mercer Admin check Internal Only ' + td_content.ArticleType + ' on ' + td_content.ArticleType + ' details page', function () {
        ...
        it('AND: I see the internal only ' + td_content.ArticleType + ' header card with a tag is: ' + td_content.Type.toUpperCase(), function () {
            fcommon_test.__checkElement_getText(presearch_details_page._tag_headerTag, td_content.Type.toUpperCase())
        })
        ...
        it('AND: I see the internal only ' + td_content.ArticleType + ' Disclaimer is displayed', function () {
            fcommon_test.__checkElement_isDisplayed(pcommon_page._txtDisclaimer, true)
        })

        it('AND: I see the internal only ' + td_content.ArticleType + ' video is displayed', function () {
            fcommon_test.__checkElement_isDisplayed(pcommon_page._lnlVideo, true)
            fcommon_test.__checkElement_attributeValue(pcommon_page._lnlVideo, 'src', td_content.VideoLink_embed)
        })
        ...
        it('AND: I see the attachment is under https://docs.src.us-east-1.dev.int.mercer.com/v1/api/proxy/document/', function () {
            fcommon_test.__checkInternalOnlyAttachment(pcommon_page._lnkAttachment)
        })

    })

    // describe('Verify Mercer Admin check Ralated Internal Only ' + td_content.ArticleType + ' on ' + td_content.ArticleType + ' page', function () {

    //     it('THEN: I see the Ralated internal only ' + td_content.ArticleType + ' card with a blue background color', function () {
    //         fcommon_test.__checkInternalOnlyArticle_backGroundColor(pcommon_page._cardArticle(popularArticle_content.title))
    //     })

    //     it('AND: I see the Ralated internal only ' + td_content.ArticleType + ' card with a tag "Intrnal-Only"', function () {
    //         fcommon_test.__checkElement_isDisplayed(pcommon_page._tagInternalOnly_inArticleCard(popularArticle_content.txtInternalOnlyTag.toUpperCase()), true)
    //     })

    //     it('AND: I see the Ralated internal only ' + td_content.ArticleType + ' card with a tag is: ' + popularArticle_content.Type, function () {
    //         fcommon_test.__checkElement_getText(presearch_page._tag_inArticleCard(popularArticle_content.title), popularArticle_content.Type.toUpperCase())
    //     })

    //     it('AND: I see the Ralated internal only ' + td_content.ArticleType + ' card title is: ' + popularArticle_content.title, function () {
    //         fcommon_test.__checkElement_getText(pcommon_page._txtTitle_inArticleCard(popularArticle_content.title), popularArticle_content.title)
    //     })

    //     it('AND: I see the Ralated internal only ' + td_content.ArticleType + ' card Executive Summary is: ' + popularArticle_content.ExecutiveSummary, function () {
    //         fcommon_test.__checkElement_getText(pcommon_page._txtExecutiveSummary_inArticleCard(popularArticle_content.ExecutiveSummary), popularArticle_content.ExecutiveSummary)
    //     })

    //     it('AND: I see the Ralated internal only ' + td_content.ArticleType + ' card Company Name is: ' + popularArticle_content.company, function () {
    //         fcommon_test.__checkElement_getText(pcommon_page._txtCompanyName_inArticleCard(popularArticle_content.title), popularArticle_content.company)
    //     })

    //     it('AND: I see the internal only ' + td_content.ArticleType + ' card Read Time is: ' + popularArticle_content.readTime, function () {
    //         fcommon_test.__checkElement_getText(pcommon_page._txtReadTime_inArticleCard(popularArticle_content.title), popularArticle_content.readTime + ' min read')
    //     })

    //     it('AND: I see the internal only ' + td_content.ArticleType + ' card view icon is displayed', function () {
    //         fcommon_test.__checkElement_isDisplayed(pcommon_page._iconView_inArticleCard(popularArticle_content.title), true)
    //     })

    //     it('AND: I see the internal only ' + td_content.ArticleType + ' card view count is displayed', function () {
    //         fcommon_test.__checkElement_isDisplayed(pcommon_page._txtViewCount_inArticleCard(popularArticle_content.title), true)
    //     })

    //     it('AND: I see the internal only ' + td_content.ArticleType + ' card rating star is displayed', function () {
    //         fcommon_test.__checkElement_isDisplayed(pcommon_page._iconStar1_inArticleCard(popularArticle_content.title), true)
    //         fcommon_test.__checkElement_isDisplayed(pcommon_page._iconStar2_inArticleCard(popularArticle_content.title), true)
    //         fcommon_test.__checkElement_isDisplayed(pcommon_page._iconStar3_inArticleCard(popularArticle_content.title), true)
    //         fcommon_test.__checkElement_isDisplayed(pcommon_page._iconStar4_inArticleCard(popularArticle_content.title), true)
    //         fcommon_test.__checkElement_isDisplayed(pcommon_page._iconStar5_inArticleCard(popularArticle_content.title), true)
    //     })

    //     it('AND: I see the internal only ' + td_content.ArticleType + ' card rating is displayed', function () {
    //         fcommon_test.__checkElement_isDisplayed(pcommon_page._txtRating_inArticleCard(popularArticle_content.title), true)
    //     })

    //     it('AND: I see the internal only ' + td_content.ArticleType + ' card bookmark button is displayed', function () {
    //         fcommon_test.__checkElement_isDisplayed(pcommon_page._btnBookmark_inArticleCard(popularArticle_content.title), true)
    //     })
