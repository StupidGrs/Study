...
let td_user = {
    ...
    "globAdmin": browser.params.login.login_globAdmin,
}
...
let td_content = {
    ...
    "Title": '2512_AutoResearch_extNormal_' + futil_timer.__returnYYYYMMDDHMS(),
...
let popularArticle_content = {
    ...
    "title": 'Research_intNormal_notRemove_2512',
...
describe('Verify External User check normal ' + td_content.ArticleType, function () {

    describe('Verify External User check normal ' + td_content.ArticleType + ' on ' + td_content.ArticleType + ' page', function () {

        it('WHEN: External User login as: ' + td_user.globAdmin, function () {
            fcommon_test.__GivenTheUserHasLoginSRC(td_user.globAdmin)
        })

        it('THEN: I see the normal ' + td_content.ArticleType + ' in ' + td_content.ArticleType + ' page: ' + td_content.Title, function () {
        ...
        it('AND: I see the normal ' + td_content.ArticleType + ' card without a tag "Internal-Only"', function () {
    ...
    describe('Verify External User check popular normal ' + td_content.ArticleType + ' on ' + td_content.ArticleType + ' page', function () {
    ...
    describe('Verify External User check normal ' + td_content.ArticleType + ' on ' + td_content.ArticleType + ' details page', function () {
        ...
        it('AND: I see the normal ' + td_content.ArticleType + ' has not a tag ' + txtInternalOnlyTag, function () {
            fcommon_test.__checkElement_isDisplayed(presearch_details_page._tag_detailsPage(txtInternalOnlyTag), false)
        })

        it('AND: I see the attachment is under https://docs.src.us-east-1.dev.int.mercer.com/v1/api/proxy/document/', function () {
            fcommon_test.__checkNormalAttachment(presearch_details_page._lnkAttachment)
        })
