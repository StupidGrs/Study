...
let td_content = {
    ...
    "Title": '0_AutoResearch_Draft_' + futil_timer.__returnYYYYMMDDHMS(),
...
describe('Verify SRC publisher check file upload with different document types on Posts - research draft page', function () {

    it('WHEN User login as: ' + td_user.compAuthor, function () {
        fcommon_test.__GivenTheUserHasLoginSRC(td_user.compAuthor)
    })

    it('AND I create a Research draft - ' + td_content.Title, function () {
        fcommon_test.__UserSaveAnArticleOnPublishPageWithMandatoryFields(td_content, true)
    })

    it('AND I see Posts - Research draft opened', function () {
