  ...
  <body>
    ...
    <div id="transDevotion" class="trans-wrapper showDevotion">
		<div id="ugcReportButton">
	        <p class='devotionTip'>
				<span class="sp wordTrans contributeBtn"></span>
				<a href="javascript:void(0);" class="tipDevotion contributeBtn">如果你找到了答案，请贡献本词条释义，利人利己</a>
			</p>
		</div>
		<div id="ugcContentEdit">
			<h3 class="devotionTip"><span class="tab-current"><span>释义贡献</span></span><div class="tail"></div><a class="toggle toggleOpen" rel="#transformToggleUgc"></a></h3>
			<div id="transformToggleUgc" class="trans-container devotionTip">
				<p id="ugcCotent"></p>
				<p class="additional"><a href="#" class="returnEdit">返回编辑</a></p>
			</div>
		</div>
        <form class="trans-container edit">
           <p><label for="currentWord">当前词条：</label><strong id="currentWord"></strong> </p>
           <p><label for="inputTrans" class="center">输入释义：<span class="required">*</span></label><textarea id="inputTrans" class="trans-content" name="inputTrans"></textarea> <span class="error-message"></span></p>
           <p><label for="source">来源参考：</label><input type="text" id="source" name="source" class="trans-message"/></p>
           <p><label for="contact">联系方式：</label><input type="text" id="contact" name="contact" class="trans-message"/></p>

           <p class="submitTip"><a class="sp ensure" href="#" title="确定"></a><a class="sp cancel" href="javascript:void(0)" title="取消"></a></p>
        </form>
    </div>
		</xsl:if>
	</xsl:if>
        ...
        <xsl:if test="yodaodict/thirdpart-result/fanyi-result">
          <div id="fanyiResult" class="trans-wrapper">
            <h3>
              ...
              <div class="tail"></div>
              <a class="toggle toggleOpen" rel="#fanyiformToggle"></a>
            </h3>
              ...
              <div class="trans-container">
                ...
                <p class="additional">
                  长段文字建议使用<xsl:element name="a">
                    ...
                    <xsl:attribute name="style">text-decoration:none</xsl:attribute><xsl:attribute name="href">
                      app:trans:<xsl:value-of select="yodaodict/input" disable-output-escaping="yes"/>
                    </xsl:attribute>有道翻译</xsl:element>，翻译结果仅供参考
                </p>
              </div>
            </div>
          </div>
	  ...
	  <xsl:if test="(/yodaodict/basic/dict) or (/yodaodict/basic/simple-dict)">
		<div id="reportError" class="trans-container">
			<h4 class="tipBack"><span class="sp tipDo"></span><span class="tipDevotion">我要报错</span></h4>

			<form>
				<ul class="optionsLeft" >
					<li><input type="checkbox" name="wordError" id="wordError" class="trans-message" /><label id="wordErrorLabel" for="wordError">本词条拼写有误</label></li>
					<li><input type="checkbox" name="phoneticError" id="phoneticError" class="trans-message" /><label id="phoneticErrorLabel" for="phoneticError">发音/音标有误</label></li>
					<li><input type="checkbox" name="others" id="others" class="trans-message" /><label id="othersLabel" for="others">其他</label></li>
				</ul>
				<ul class="optionsRight" >
					<li><input type="checkbox" name="transError" id="transError" class="trans-message" /><label id="transErrorLabel" for="transError">释义有误</label></li>
					<li><input type="checkbox" name="sentencesError" id="sentencesError" class="trans-message" /><label id="sentencesErrorLabel" for="sentencesError">例句有误</label></li>
				</ul>
				<h5 class="clearFloat">错误描述</h5>
				<textarea id="inputRepoertError" class="trans-content"></textarea>
				<span class="error-message"></span>

				<p class="alignCenter"><a class="sp ensure" href="#" title="确定"></a><a class="sp cancel" href="javascript:void(0)" title="取消"></a></p>
			</form>
		</div>
	  </xsl:if>


      <div id ="phrsListTab" class ="trans-wrapper dontRemember"> 
    ...
    <xsl:if test="not(/yodaodict/basic/simple-dict) and not(/yodaodict/thirdpart-result)">
            ...
            <xsl:if test="/yodaodict/basic">
              ...
              <xsl:for-each select="/yodaodict/basic">
                <xsl:if test="type[text()!='ee']" >
                  <div id="simple_dict_trans"  class="trans-container tab-content" style="display:none">
                    <!-- 显示tab -->
                    <!-- 输出简明词典 -->
      ...
      <xsl:if test="yodaodict/yodao-web-dict/web-translation">
        <table id="yodao_anchor_phrase" cellpadding="0" cellspacing="0" border="0">
          <tr>
            <td height="0px" style="font-size:0"></td>
          </tr>
        </table>
        ...
        <div id="webTrans" class="trans-wrapper">
          <h3>
            <span class="tabs">
              ...
              <xsl:if test="yodaodict/yodao-web-dict/web-translation">
                <xsl:element name="a">
                  ...
                  <xsl:attribute name="onclick">
                    ...
                    <xsl:variable name="danyinhao2">ba</xsl:variable>
              ...
              <xsl:for-each select="/yodaodict/basic">
                <xsl:if test="type[text()!='ee']" >
                  <xsl:if test="simple-dict">
                    <xsl:if test="authoritative-dict">
                      <xsl:element name="a">
                        <xsl:attribute name="rel">#tPowerTrans</xsl:attribute>
                        <xsl:attribute name="onclick">
            ...
            <div class="tail"></div>
            <a class="toggle toggleOpen" rel="#webtransformToggle"></a>
          </h3>
          <div id="webtransformToggle">
                        ...
                        <xsl:when test="position() &lt; 5">
                          <p class="wordGroup">
                            <span class="contentTitle">
                              <xsl:element name="a">
                                <xsl:attribute name="href">
                                  app:ds:<xsl:value-of select="./key"/>
                                </xsl:attribute>
                                <xsl:attribute name="target">_self</xsl:attribute>
            ...
            <xsl:for-each select="/yodaodict/basic">
              <xsl:if test="type[text()!='ee']" >
                <xsl:if test="simple-dict">
                  <xsl:if test="authoritative-dict">
                    <div id="tPowerTrans" class="trans-container tab-content">
                      <xsl:apply-templates select="authoritative-dict"/>
                    </div>
                  </xsl:if>
                </xsl:if>
      ...
      <table id="yodao_anchor_sentence" cellpadding="0" cellspacing="0" border="0">
        <tr>
          <td height="1px" style="font-size:0"></td>
        </tr>
      </table>
        ...
        <xsl:if test="yodaodict/thirdpart-result/websearch-result">
          <div id="wsResult" class="trans-wrapper">
            ...
            <h3>
              ...
              <div class="tail"></div>
              <a class="toggle toggleOpen" rel="#webSearchResults"></a>
            </h3>
        ...
        <xsl:if test="/yodaodict/baike">
          ...
          <div id="eBaike" class="trans-wrapper">
            ...
            <h3>
              ...
              <div class="tail"></div>
              <a class="toggle toggleOpen" rel="#baikeTab"></a>
            </h3>
            <div id="baikeTab">
              <div class="trans-container">
                <div id="baike">
          ...
          <xsl:for-each select="yodaodict/local-dicts/dict">
            ...
            <xsl:if test="(word[text()!=''])">
              <table cellpadding="0" cellspacing="0" border="0">
                <xsl:attribute name="id">yodao_anchor_custom<xsl:number value="position()"/>
                </xsl:attribute>
                <tr>
                  <td height="1px" style="font-size:0"></td>
                </tr>
              </table>
              <div id="ecTrans_local" class="trans-wrapper dontRemember">
                <!--循环显示每部字典的内容-->
                <h3>
                  <xsl:choose>
                    ...
                    <xsl:otherwise>
                      <span class ="tab-current">
                        <a>
                          <xsl:attribute name="rel">#simple_dict_trans_local<xsl:number value="position()"/>
                          </xsl:attribute>
                          <xsl:attribute name="onclick">
                            ctlog('', '' , 0, 'deskdict.main' , 1, 'CLICK',  'tab切换基本释义');
                          </xsl:attribute>
                ...
                <div >
                  ...
                  <div class="trans-container tab-content">
                    <xsl:attribute name="id">simple_dict_trans_local<xsl:number value="position()"/>
                    </xsl:attribute>
                    ...
                    <xsl:if test="transforms/transform">
                      <p class="additional">
                        ...
                        <xsl:for-each select="transforms/transform">
                          <xsl:value-of select="type" /> <xsl:value-of select="word" /> &#000032;
                        </xsl:for-each> ]
            ...
            <xsl:if test="authoritative-dict">
              ...
              <div id="ecTrans_local_auth" class="trans-wrapper">
                ...
                <h3>
                  ...
                  <div class="tail"></div>
                  <a class="toggle toggleOpen" rel="#authTrans"></a>
                </h3>
		...
		<!-- 显示一条黑色的横线 -->
          ...
          <xsl:if test="not(yodaodict/basic/simple-dict) and not(yodaodict/basic/dict)">
            <xsl:if test="yodaodict/basic/authoritative-dict">
              <li id="btsource">
                <xsl:value-of select="yodaodict/basic/name"/>基本释义来源于:
          ...
          <xsl:if test="yodaodict/baike">
            <li id="btsource">
              ...
              <xsl:element name="a">
                <xsl:attribute name="href">
                  <xsl:value-of select="yodaodict/baike/source/url"/>
                </xsl:attribute>
                <xsl:attribute name="class">bottomsource</xsl:attribute>
                <xsl:attribute name="target">_blank</xsl:attribute>
                <xsl:attribute name="onclick">
          ...
          <xsl:if test="yodaodict/sentences-result/media-sents/sent">
            <li id="btsource">部分原声例句来源于<a href="http://yyets.com">人人影视</a>、<a href="http://bbs.uuniao.com">悠悠鸟</a>、<a href="http://bbs.sfileydy.com">伊甸园</a>等 对外免费发布的字幕素材</li>
          </xsl:if>
        </ul>
	</xsl:if>
	<!-- 简明翻译来源的声明 -->
	...
	<xsl:if test="not(yodaodict/basic)">
		...
		<div class="trans-wrapper" ><!-- it's a work-around  -->
			没有找到与您查询的"<b>
			...
			</b>"相关的<xsl:call-template name="get_language_name"><xsl:with-param name="lang_type" select="/yodaodict/language" /></xsl:call-template>结果。
...
<div id ="phrsListTab" class ="trans-wrapper"> 
<xsl:call-template name="ead_block">
