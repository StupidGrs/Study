...
<xsl:template name="single_tab">
  <xsl:param name="tab_title"/>
     <h3><span class="tab-current"><span><xsl:value-of select="$tab_title"/></span></span><div class="tail"></div></h3>
</xsl:template>
	...
	<xsl:for-each select="word">
		...
		<div id="classic" >
			...
			<xsl:call-template name="keyword_line">
				...
				<xsl:with-param name="phone" select="phone"/>
				<xsl:with-param name="phonesup" select="phonesup"/>
        <xsl:with-param name="speech" select="/no_such_item"/> 
				<xsl:with-param name="field" select="field"/>
				<xsl:with-param name="origin" select="origin"/>
				<xsl:with-param name="showadd2wordbook">false</xsl:with-param>
			</xsl:call-template>
...
<xsl:template name="web-translation-first">
	...
	<div class="wt-container wt-collapse">
	  <div>
        <a href="#" title="详细释义" rel="#rw1" class="sp do-detail" style="cursor: pointer;">
          <xsl:attribute name="onclick">
    ...
    <xsl:choose>
      <xsl:when test="name()='chars'">
        <span>
          ...
          <xsl:attribute name="onmouseover">LjHighlightAlign(&apos;<xsl:value-of select="$highlight_str"/>&apos;)</xsl:attribute>
          <xsl:attribute name="onmouseout">LjUnhighlightAlign(&apos;<xsl:value-of select="$highlight_str"/>&apos;)</xsl:attribute>
          <xsl:choose>
            <xsl:when test="q">
  ...
  <xsl:template name="sentence_result">
    ...
    <xsl:param name="inputEncode"/>
        <div id="examples" class="trans-wrapper">
          ...
          <xsl:attribute name="onclick">
            ctlog('', '<xsl:value-of select="$input"/>' , 0, 'deskdict.main.morelj.blng' , 1, 'CLICK',  '点击更多双语例句');
        </xsl:attribute>
        更多双语例句
      </xsl:element>
    </xsl:if>
  ...
  <div id ="yuansheng" class="trans-container tab-content">
    <xsl:if test="media-sents/sent">
      ...
      <xsl:element name="a">
        ...
        <xsl:attribute name="onclick">
          ctlog('', '<xsl:value-of select="$input"/>' , 0, 'deskdict.main.morelj.mdia' , 1, 'CLICK',  '点击更多原声例句');
        </xsl:attribute>
        更多原声例句
      </xsl:element>
    </xsl:if>
  ...
  <div id ="quanwei" class="trans-container tab-content">
    <xsl:if test="auth-sents/sent">
      ...
      <xsl:element name="a">
        ...
        <xsl:attribute name="onclick">
          ctlog('', '<xsl:value-of select="$input"/>' , 0, 'deskdict.main.morelj.auth' , 1, 'CLICK',  '点击更多权威例句');
        </xsl:attribute>
				更多权威例句
			  </xsl:element>
			   </xsl:if>
