@events
Feature: Verify all elements and fields on Draft Event Page, when Event saved with required fields only
    ...
    Background:
        ...
        And User "COMPANY_ADMIN" saves Draft Event with title "$eventTitle" and Start Date "10/20/2020" and Time "04:10 PM" and End Date "10/20/2021" and Time "05:10 PM" with API
    ...
    @saveDraftState
    @dirtyChecking
    Scenario: Verify Draft Event Page, when Event saved with required fields only
        #Header
...
#        Then Region options list "createEventPage|regionOptionRow" is displayed
        And User verifies each Region checkbox "createEventPage|regionOptionCheckboxInput" item is not selected
        #Start Date
        ...
        And URL link field "createEventPage|urlLinkField" input text is equal to ""
        ...
        And Tag chip "createEventPage|tagChipItem" is not displayed
        #Excerpt
...
#        And Uploaded Image "createEventPage|attachedImageLabel" is not displayed
        #Buttons
