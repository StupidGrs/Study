@events
Feature:  Global Admin updates all fields of Pending Event and clicks [Approve] / [Approve and open the next] button

  Background:
    Given User "COMPANY_ADMIN" logs in with API
    And User remembers text "Test_Auto_Event" with added unique Id as "eventTitle"
    And User "COMPANY_ADMIN" publishes "Event" with all fields and title "$eventTitle" with API
    ...
    #    remove: And User remembers value of "value" attribute of "moderateEventPage|eventStartDateInput" as "updatedStartDate"
...
#    And User clicks Remove Featured Image button "moderateEventPage|eventRemoveFeaturedImageButton"
  ...
  Scenario: Global Admin updates all fields, clicks [Approve] button and verifies updated values
    And User clicks [Approve] button "moderateEventPage|approveButton"
    ...
    And User enters "$updatedEventTitle" in Search Content field "contentListPage|searchContentField"
    Then User waits for Table Row "contentListPage|tableRowsList" with text "$updatedEventTitle" visibility within 3 seconds
    And User clicks Title "contentListPage|titleList" on Table Row "contentListPage|tableRowsList" with text "$updatedEventTitle"
    #Verify all fields
    And User scrolls page to top
    And Event Status "moderateEventPage|articleStatus" with text "Status: APPROVED" is displayed
  ...
  Scenario: Global Admin updates all fields, clicks [Approve and open the next] button and verifies updated values
    When User clicks [Approve and open the next] button "moderateEventPage|approveAndOpenNextButton"
    #Check that the next Pending Event is opened
    Then User waits 2 seconds
    And User scrolls page to top
    And Page Header "moderateEventPage|pageHeader" with text "Moderate event" is displayed
    #Sometimes it opens already updated article, so status can differ
    #And Article Status "moderateEventPage|articleStatus" with text "Status: PENDING" is displayed
    #And Attribute "value" of Title field "moderateEventPage|titleField" is not equal to "$updatedEventTitle"
